...
$(document).ready(function() {
	...
	// AANGEPASTE CODE
	// Volgorde van analisys aanpassen
function reorderAnalysisColumns() {
    const table = document.querySelector('#model-relations');
    if (!table) return;

    const headerRow = table.querySelector('thead tr:nth-child(2)');
    const rows = table.querySelectorAll('tbody tr');

    // Current Archi export order: name, type, source, target
    const thName = headerRow.children[0];
    const thType = headerRow.children[1];
    const thSource = headerRow.children[2];
    const thTarget = headerRow.children[3];

    // New order: source, name, target, type
    ...
    rows.forEach(row => {
        ...
        const tdName = cells[0];
        const tdType = cells[1];
        const tdSource = cells[2];
        const tdTarget = cells[3];
...
function hideTypeColumn() {
    const table = document.querySelector('#model-relations');
    if (!table) return;

    // Find the Type header by its unique class
    const typeHeader = table.querySelector('th.i18n-type');
    if (!typeHeader) return;

    // Determine index of the Type column
    const columnIndex = Array.from(typeHeader.parentElement.children).indexOf(typeHeader);

    // Hide header cell
    typeHeader.style.display = "none";

    // Hide all data cells in the same column
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach(row => {
